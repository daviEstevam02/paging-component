...

{
    ...
    public async Task<CommandResult> Handle(CreateUserDocumentCommand command, CancellationToken cancellationToken)
    {
        ...
        return await Commit(_userDocumentRepository.UnitOfWork, "Tipo de usuario documento criado com sucesso!");

    }

    public Task<CommandResult> Handle(UpdateUserDocumentCommand request, CancellationToken cancellationToken)
    {
